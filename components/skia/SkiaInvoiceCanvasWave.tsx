...
import { Canvas, Rect, Text, Skia, matchFont, Circle, Paragraph, TextAlign, Image, useImage, Group, Path, LinearGradient, vec, RoundedRect } from '@shopify/react-native-skia';
...
const SkiaInvoiceCanvasWave = forwardRef((props: SkiaInvoiceCanvasProps, ref: any) => {
  const { 
    invoice, 
    business, 
    client, 
    ...
    renderSinglePage,
    exportPageNumber,
    accentColor = '#8B5CF6', // Default purple for wave
  ...
  const totalCanvasHeight = exportPageNumber ? 560 : // Use export height if in export mode
  ...
  const rightAlignedParagraphs = useMemo(() => {
    ...
    try {
      ...
      .pushStyle({ 
        color: Skia.Color('white'), // White text for wave header
        fontFamilies: ['Helvetica'], 
        fontSize: 20, // Smaller than Modern's 32px
  ...
  // Wave design colors - using gradient shades of accent color
  const colors = {
    background: '#fff',
    text: 'black',
    border: '#eee',
    waveGradientStart: accentColor, // Main accent color
    waveGradientEnd: `${accentColor}CC`, // Slightly lighter shade
    shadow: '#f0f0f0',
    lightAccent: accentColor ? `${accentColor}15` : '#8B5CF615' // Very light version of accent color (15% opacity in hex)
  };
    ...
    <View style={[
      ...
      <View style={[
        ...
        >
          ...
          {/* === WAVE HEADER DESIGN === */}
          {/* Create seamless flowing wave with upward curve on left */}
          <Path
            path={(() => {
              const path = Skia.Path.Make();
              path.moveTo(0, 0);
              path.lineTo(0, 15); // Start thicker on left
              
              // ONE seamless curve - LEFT SIDE curves UP (toward top)
              path.cubicTo(
                canvasWidth * 0.15, 5,   // Control point 1 - curve UP toward top (smaller y)
                canvasWidth * 0.35, 90,  // Control point 2 - then dip down dramatically
                canvasWidth * 0.6, 85    // Control point 3 - maintain depth
              );
              
              path.cubicTo(
                canvasWidth * 0.8, 80,   // Control point 4 - continue smooth flow
                canvasWidth * 0.95, 75,  // Control point 5 - approach end smoothly
                canvasWidth, 85          // End point - thick on right
              );
          ...
          >
            <LinearGradient
              start={vec(0, 0)}
              end={vec(canvasWidth, 80)}
              colors={[colors.waveGradientStart, colors.waveGradientEnd]}
            />
          </Path>
          ...
          {rightAlignedParagraphs && (
            <>
              {/* Smaller right-aligned INVOICE title - Moved up 10px */}
              <Paragraph paragraph={rightAlignedParagraphs.invoiceParagraph} x={OFFSET_X + 180} y={15} width={190} />
              {/* Business name under INVOICE in header - Moved up 10px */}
              <Paragraph paragraph={rightAlignedParagraphs.businessNameParagraph} x={OFFSET_X + 180} y={40} width={190} />
              {/* Business address under business name in header - Moved up 10px */}
              <Paragraph paragraph={rightAlignedParagraphs.businessAddressHeaderParagraph} x={OFFSET_X + 180} y={60} width={190} />
            </>
          )}
          ...
          <RoundedRect
            ...
            y={193}
            width={tableWidth - 35}
            height={(actualFirstPageItems.length * scaledRowHeight) + 22} // Dynamic height based on items
            r={8} // 8px border radius
            color={colors.background}
          />
          
          {/* Table header with accent color - rounded top corners */}
          <RoundedRect
            x={tableX + 15}
            y={195}
            width={tableWidth - 35}
            height={18}
            r={8} // Will only show top corners due to overlap
            color={colors.waveGradientStart}
          />
          ...
          {/* Line items for the first page with alternating background colors */}
              ...
              pages.push(
                <React.Fragment key={`page-${currentPage}`}>
                  ...
                  <RoundedRect
                    ...
                    y={pageHeaderY - 2}
                    width={tableWidth - 35}
                    height={(itemsThisPage.length * scaledRowHeight) + 22}
                    r={8}
                    color={colors.background}
                  />
                  ...
                  {itemsThisPage.map((item: any, index: number) => {
                    ...
                    return (
                      <React.Fragment key={`page-${currentPage}-item-${index}`}>
                        ...
                        <Paragraph paragraph={priceParagraph} x={priceX + 65} y={rowY + textOffsetY - 2} width={80} />
                        <Paragraph paragraph={totalParagraph} x={totalX + 62} y={rowY + textOffsetY - 2} width={80} />
                      </React.Fragment>
                    );
                  })}
            ...
            if (!actualNeedsPagination || (exportPageNumber && showExportFooter)) {
              ...
              return (
                <>
                  ...
                  {(invoice?.gocardless_active || invoice?.stripe_active || invoice?.paypal_active || invoice?.bank_account_active) && (
                    <>
                      ...
                      {invoice?.stripe_active && (
                        <>
                          <Text x={27} y={paymentMethodsY + 14 + (invoice?.gocardless_active ? 26 : 0) + (invoice?.paypal_active ? 38 : 0)} text="Pay Online" font={scaledFonts.body} color={colors.text} />
                          ...
                          {visaIcon && (
                            <Image
                              image={visaIcon}
                              x={85}
                              y={paymentMethodsY + 6 + (invoice?.gocardless_active ? 26 : 0) + (invoice?.paypal_active ? 38 : 0)}
                              width={24}
                              height={14}
                              fit="contain"
                            />
                          ...
                          <Text x={27} y={paymentMethodsY + 26 + (invoice?.gocardless_active ? 26 : 0) + (invoice?.paypal_active ? 38 : 0)} text="www.stripelink.com" font={scaledFonts.body} color={colors.text} />
                        </>
                      )}
                  ...
                  {/* Totals section with rounded corners */}
                  <RoundedRect
                    x={totalX - 30}
                    y={footerY + 10}
                    width={134}
                    height={(hasDiscount ? 95 : 75) + (rightAlignedParagraphs?.paidLabelParagraph ? 40 : 0) + 25}
                    r={8}
                    color={colors.shadow}
                  />
                  ...
                  {rightAlignedParagraphs ? (
                    <>
                      {/* Subtotal row */}
                      <Paragraph paragraph={rightAlignedParagraphs.subtotalLabelParagraph} x={totalX - 24} y={footerY + 15} width={70} />
                      <Paragraph paragraph={rightAlignedParagraphs.subtotalValueParagraph} x={totalX + 36} y={footerY + 15} width={60} />
                      
                      {/* Discount row (conditional) */}
                      {rightAlignedParagraphs.discountLabelParagraph ? (
                        <>
                          <Paragraph paragraph={rightAlignedParagraphs.discountLabelParagraph} x={totalX - 24} y={footerY + 35} width={70} />
                          <Paragraph paragraph={rightAlignedParagraphs.discountValueParagraph} x={totalX + 36} y={footerY + 35} width={60} />
                        </>
                      ) : null}
                      
                      {/* Tax row - adjusts position based on discount presence */}
                      <Paragraph paragraph={rightAlignedParagraphs.taxLabelParagraph} x={totalX - 24} y={footerY + (hasDiscount ? 55 : 35)} width={70} />
                      <Paragraph paragraph={rightAlignedParagraphs.taxValueParagraph} x={totalX + 36} y={footerY + (hasDiscount ? 55 : 35)} width={60} />
                      
                      {/* Paid row (conditional - between VAT and Balance Due) */}
                      {rightAlignedParagraphs.paidLabelParagraph ? (
                        <>
                          <Paragraph paragraph={rightAlignedParagraphs.paidLabelParagraph} x={totalX - 24} y={footerY + (hasDiscount ? 75 : 55)} width={70} />
                          <Paragraph paragraph={rightAlignedParagraphs.paidValueParagraph} x={totalX + 36} y={footerY + (hasDiscount ? 75 : 55)} width={60} />
                          
                          {/* Balance Due row (directly under Paid) */}
                          <Paragraph paragraph={rightAlignedParagraphs.balanceDueLabelParagraph} x={totalX - 24} y={footerY + (hasDiscount ? 95 : 75)} width={70} />
                          <Paragraph paragraph={rightAlignedParagraphs.balanceDueValueParagraph} x={totalX + 36} y={footerY + (hasDiscount ? 95 : 75)} width={60} />
                        </>
                      ) : null}
                    </>
                  ) : null}
                  
                  {/* Grand Total Box with wave gradient background - positioned after Balance Due line if present */}
                  <RoundedRect
                    x={totalX - 26}
                    y={footerY + (hasDiscount ? 95 : 75) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)}
                    width={129}
                    height={20}
                    r={8}
                    color={colors.waveGradientStart}
                  ...
                  {rightAlignedParagraphs ? (
                    <>
                      <Paragraph paragraph={rightAlignedParagraphs.totalLabelParagraph} x={totalX - 24} y={footerY + (hasDiscount ? 100 : 80) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)} width={70} />
                      <Paragraph paragraph={rightAlignedParagraphs.totalValueParagraph} x={totalX + 36} y={footerY + (hasDiscount ? 100 : 80) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)} width={65} />
                    </>
                  ) : null}
                ...
                <>
                  ...
                  {(invoice?.gocardless_active || invoice?.stripe_active || invoice?.paypal_active || invoice?.bank_account_active) && (
                    <>
                      ...
                      {invoice?.stripe_active && (
                        <>
                          <Text x={27} y={lastPagePaymentMethodsY + 14 + (invoice?.gocardless_active ? 26 : 0) + (invoice?.paypal_active ? 38 : 0)} text="Pay Online" font={scaledFonts.body} color={colors.text} />
                          ...
                          {visaIcon && (
                            <Image
                              image={visaIcon}
                              x={85}
                              y={lastPagePaymentMethodsY + 6 + (invoice?.gocardless_active ? 26 : 0) + (invoice?.paypal_active ? 38 : 0)}
                              width={24}
                              height={14}
                              fit="contain"
                            />
                          ...
                          <Text x={27} y={lastPagePaymentMethodsY + 26 + (invoice?.gocardless_active ? 26 : 0) + (invoice?.paypal_active ? 38 : 0)} text="www.stripelink.com" font={scaledFonts.body} color={colors.text} />
                        </>
                      )}
                  ...
                  {/* Totals section with rounded corners on last page */}
                  <RoundedRect
                    x={totalX - 30}
                    y={lastPageFooterY + 10}
                    width={134}
                    height={(hasDiscount ? 95 : 75) + (rightAlignedParagraphs?.paidLabelParagraph ? 40 : 0) + 25}
                    r={8}
                    color={colors.shadow}
                  ...
                  {rightAlignedParagraphs ? (
                    <>
                      {/* Subtotal row */}
                      <Paragraph paragraph={rightAlignedParagraphs.subtotalLabelParagraph} x={totalX - 24} y={lastPageFooterY + 15} width={70} />
                      <Paragraph paragraph={rightAlignedParagraphs.subtotalValueParagraph} x={totalX + 36} y={lastPageFooterY + 15} width={60} />
                      
                      {/* Discount row (conditional) */}
                      {rightAlignedParagraphs.discountLabelParagraph ? (
                        <>
                          <Paragraph paragraph={rightAlignedParagraphs.discountLabelParagraph} x={totalX - 24} y={lastPageFooterY + 35} width={70} />
                          <Paragraph paragraph={rightAlignedParagraphs.discountValueParagraph} x={totalX + 36} y={lastPageFooterY + 35} width={60} />
                        </>
                      ) : null}
                      
                      {/* Tax row - adjusts position based on discount presence */}
                      <Paragraph paragraph={rightAlignedParagraphs.taxLabelParagraph} x={totalX - 24} y={lastPageFooterY + (hasDiscount ? 55 : 35)} width={70} />
                      <Paragraph paragraph={rightAlignedParagraphs.taxValueParagraph} x={totalX + 36} y={lastPageFooterY + (hasDiscount ? 55 : 35)} width={60} />
                      
                      {/* Paid row (conditional - between VAT and Balance Due) */}
                      {rightAlignedParagraphs.paidLabelParagraph ? (
                        <>
                          <Paragraph paragraph={rightAlignedParagraphs.paidLabelParagraph} x={totalX - 24} y={lastPageFooterY + (hasDiscount ? 75 : 55)} width={70} />
                          <Paragraph paragraph={rightAlignedParagraphs.paidValueParagraph} x={totalX + 36} y={lastPageFooterY + (hasDiscount ? 75 : 55)} width={60} />
                          
                          {/* Balance Due row (directly under Paid) */}
                          <Paragraph paragraph={rightAlignedParagraphs.balanceDueLabelParagraph} x={totalX - 24} y={lastPageFooterY + (hasDiscount ? 95 : 75)} width={70} />
                          <Paragraph paragraph={rightAlignedParagraphs.balanceDueValueParagraph} x={totalX + 36} y={lastPageFooterY + (hasDiscount ? 95 : 75)} width={60} />
                        </>
                      ) : null}
                    </>
                  ) : null}
                  
                  {/* Grand Total Box with wave gradient background on last page */}
                  <RoundedRect
                    x={totalX - 26}
                    y={lastPageFooterY + (hasDiscount ? 95 : 75) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)}
                    width={129}
                    height={20}
                    r={8}
                    color={colors.waveGradientStart}
                  ...
                  {rightAlignedParagraphs ? (
                    <>
                      <Paragraph paragraph={rightAlignedParagraphs.totalLabelParagraph} x={totalX - 24} y={lastPageFooterY + (hasDiscount ? 100 : 80) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)} width={70} />
                      <Paragraph paragraph={rightAlignedParagraphs.totalValueParagraph} x={totalX + 36} y={lastPageFooterY + (hasDiscount ? 100 : 80) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)} width={65} />
                    </>
                  ) : null}
                </>
              );
...
export default SkiaInvoiceCanvasWave;
