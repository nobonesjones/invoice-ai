...
const SkiaInvoiceCanvasSimple = forwardRef((props: SkiaInvoiceCanvasProps, ref: any) => {
  const { 
    invoice, 
    business, 
    client, 
  ...
  // console.log('[SkiaInvoiceCanvasSimple] Rendering Simple Design');
  ...
  const rightAlignedParagraphs = useMemo(() => {
    ...
    try {
      ...
      const billToParagraph = Skia.ParagraphBuilder.Make({
        textAlign: TextAlign.Left, // Changed to left align to match From section
      })
      .pushStyle({ 
      ...
      .addText(`To:`) // Changed from "Bill To:" to "To:" to match "From:"
      .build();
      ...
      const businessNameParagraph = Skia.ParagraphBuilder.Make({
        textAlign: TextAlign.Left, // Changed to left align for Simple template
      })
      .pushStyle({ 
        color: Skia.Color('black'), // Changed to black for Simple template visibility
        fontFamilies: ['Helvetica'], 
        fontSize: 10, // Reduced size for Simple template
        fontStyle: { weight: 700 }
      })
      ...
      <View style={[
        ...
        >
          ...
          <Text 
            ...
            y={35} 
            text={documentTitle} 
            font={scaledFonts.title} 
            color="black" 
          />
          
          {/* Right: Business Logo aligned with invoice data section */}
          {displaySettings.show_business_logo && (
            ...
            <>
              {/* Fallback logo with dynamic business initials - aligned with right section */}
              <Circle cx={OFFSET_X + 350} cy={35} r={20} color={colors.headerBlock} />
              <Text 
                x={OFFSET_X + (businessInitials.length === 1 ? 346 : 342)} 
                y={41} 
          ...
          {/* Simple horizontal line under header - dynamic accent color */}
          <Rect x={OFFSET_X + 15} y={60} width={canvasWidth - 30} height={1} color={colors.greenAccent} />
          
          {/* === SIMPLE LAYOUT SECTIONS === */}
          
          {/* Left side: From and To sections */}
          {rightAlignedParagraphs && (
            <>
              {/* From section */}
              <Paragraph paragraph={rightAlignedParagraphs.fromLabelParagraph} x={OFFSET_X + 19} y={80} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.businessNameParagraph} x={OFFSET_X + 19} y={95} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.businessAddress1Paragraph} x={OFFSET_X + 19} y={107} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.businessAddress2Paragraph} x={OFFSET_X + 19} y={119} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.businessAddress3Paragraph} x={OFFSET_X + 19} y={131} width={150} />
              {rightAlignedParagraphs.businessTaxNumberParagraph && (
                <Paragraph paragraph={rightAlignedParagraphs.businessTaxNumberParagraph} x={OFFSET_X + 19} y={143} width={150} />
              )}
              
              {/* To section - moved up 5px */}
              <Paragraph paragraph={rightAlignedParagraphs.billToParagraph} x={OFFSET_X + 19} y={160} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.clientNameParagraph} x={OFFSET_X + 19} y={175} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.clientAddress1Paragraph} x={OFFSET_X + 19} y={187} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.clientAddress2Paragraph} x={OFFSET_X + 19} y={199} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.clientAddress3Paragraph} x={OFFSET_X + 19} y={211} width={150} />
              <Paragraph paragraph={rightAlignedParagraphs.clientTaxNumberParagraph} x={OFFSET_X + 19} y={223} width={150} />
            </>
          )}
          
          {/* Right side: Invoice data - aligned to right edge */}
          {rightAlignedParagraphs && (
            <>
              {/* Invoice Number */}
              <Paragraph paragraph={rightAlignedParagraphs.refParagraph} x={canvasWidth - 150} y={80} width={130} />
              {/* Date */}
              <Paragraph paragraph={rightAlignedParagraphs.dateParagraph} x={canvasWidth - 150} y={95} width={130} />
              {/* Due Date */}
              <Paragraph paragraph={rightAlignedParagraphs.dueParagraph} x={canvasWidth - 150} y={110} width={130} />
              {/* PO Number - conditional */}
              {rightAlignedParagraphs.poParagraph && (
                <Paragraph paragraph={rightAlignedParagraphs.poParagraph} x={canvasWidth - 150} y={125} width={130} />
              )}
            </>
          )}
          ...
          {(() => {
            ...
            if (!actualNeedsPagination || (exportPageNumber && showExportFooter)) {
              ...
              return (
                <>
                  {/* Terms section moved up to be opposite To section */}
                  {invoice?.notes && (
                    <>
                      <Text x={canvasWidth - 150} y={145} text="Terms, Instructions & Notes" font={scaledFonts.bodyBold} color={colors.text} />
                      {(() => {
                  ...
                  {/* Dynamic color bottom section - just 10px at very bottom */}
                  <Rect x={0} y={canvasHeight - 10} width={canvasWidth} height={10} color={colors.greenAccent} />
                  
                  {/* Totals section for single page - positioned above the colored strip */}
                  {rightAlignedParagraphs ? (
                    <>
                      {/* Subtotal row */}
              ...
              return (
                <>
                  ...
                  {invoice?.notes && (
                    <>
                      <Text x={canvasWidth - 150} y={lastPageYOffset + 145} text="Terms, Instructions & Notes" font={scaledFonts.bodyBold} color={colors.text} />
                      {(() => {
                        // Word wrapping function for terms/notes text
                        const wrapText = (text: string, maxWidth: number = 36) => {
                          const words = text.split(' ');
                        ...
                        return wrappedLines.map((line, index) => (
                          <Text 
                            key={index}
                            x={canvasWidth - 150} 
                            y={lastPageYOffset + 157 + (index * 12)} 
                            text={line} 
                            font={scaledFonts.body} 
                            color={colors.text} 
                          />
                  ...
                  {/* Dynamic color bottom section on last page - just 10px at very bottom */}
                  <Rect x={0} y={lastPageYOffset + canvasHeight - 10} width={canvasWidth} height={10} color={colors.greenAccent} />
...
export default SkiaInvoiceCanvasSimple;