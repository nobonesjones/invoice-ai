...
const SkiaInvoiceCanvasClean = forwardRef((props: SkiaInvoiceCanvasProps, ref: any) => {
  const { 
    invoice, 
    business, 
    client, 
  ...
  // console.log('[SkiaInvoiceCanvasClean] Rendering Clean Design');
  ...
  const fonts = useMemo(() => {
    ...
    const createFontWithFallback = (fontSize: number, fontWeight: "normal" | "bold" = "normal") => {
      ...
      try {
        ...
        console.log(`[SkiaInvoiceCanvas] CRITICAL ERROR: Font system failure`, e);
        return null;
      }
    };
  ...
  const scaledFonts = useMemo(() => {
    ...
    const createScaledFontWithFallback = (fontSize: number, fontWeight: "normal" | "bold" = "normal") => {
      ...
      try {
        ...
        // Keep critical error logs
        console.log(`[SkiaInvoiceCanvas] CRITICAL ERROR: Scaled font system failure`, e);
        return null;
      }
    };
  ...
  const rightAlignedParagraphs = useMemo(() => {
    ...
    try {
      ...
      .pushStyle({ 
        color: Skia.Color('white'), // White text for color header
      ...
      // Keep critical error logs
      console.log('[SkiaInvoiceCanvas] CRITICAL ERROR: Paragraph creation failed:', e);
      return null;
    }
    ...
    <View style={[
      ...
      <View style={[
        ...
        >
          ...
          {/* === CLEAN HEADER BLOCK === */}
          {/* Large header block spanning full width with accent color */}
          ...
          {displaySettings.show_business_logo && (
            ...
            <>
              ...
              <Circle cx={OFFSET_X + 59} cy={47} r={32} color="white" />
              <Text 
          ...
          {rightAlignedParagraphs && (
            <>
              {/* Smaller right-aligned INVOICE title - Centered in header */}
              ...
              <Paragraph paragraph={rightAlignedParagraphs.businessNameParagraph} x={OFFSET_X + 180} y={50} width={190} />
              {/* Business address under business name in header - single line */}
              <Paragraph paragraph={rightAlignedParagraphs.businessAddressHeaderParagraph} x={OFFSET_X + 180} y={70} width={190} />
            </>
          )}
          ...
          {/* Table header with accent color - Classic padding style (not full width) */}
            ...
            if (!actualNeedsPagination || (exportPageNumber && showExportFooter)) {
              ...
              return (
                <>
                  ...
                  <Rect x={totalX - 26} y={footerY + (hasDiscount ? 95 : 75) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)} width={129} height={20} color={colors.greenAccent} />
              ...
              return (
                <>
                  ...
                  <Rect x={totalX - 26} y={lastPageFooterY + (hasDiscount ? 95 : 75) + (rightAlignedParagraphs?.paidLabelParagraph ? 25 : 10)} width={129} height={20} color={colors.greenAccent} />
...
export default SkiaInvoiceCanvasClean;