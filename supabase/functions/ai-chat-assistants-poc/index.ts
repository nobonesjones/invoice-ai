...
serve(async (req)=>{
  ...
  try {
    // Initialize OpenAI. Chat Completions with tools; the Assistants API was removed 2026-08-26.
...
**CRITICAL RULE: Status changes without payment amounts will show incorrect totals on invoice documents!**`;
    ...
    // The prompt and tool schemas used to live on an OpenAI Assistant object and were
    // pushed with assistants.update(). OpenAI removed the Assistants API on 2026-08-26,
...
• Never ignore or argue with corrections - immediately acknowledge and fix them`,
    ...
    // Chat Completions keeps no server-side thread. The app sends the last 15 messages
    // as `history`; the thread id only has to stay stable for the app's chat_threads row.
    const thread = { id: threadId || `thread-${Date.now()}` };
    ...
    console.log('[Assistants POC] Conversation built:', historyMessages.length, 'history messages, thread', thread.id);
...
• "I want something professional"`;
    ...
    // Tool-calling loop: each round the model either calls tools (run them, feed the
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      const completion = await openai.chat.completions.create({
        ...
        tools: assistantConfig.tools as any,
        tool_choice: 'auto'
      });
      const reply = completion.choices?.[0]?.message;
      if (!reply) throw new Error('OpenAI returned no choices');
      const toolCalls = reply.tool_calls || [];
      if (toolCalls.length === 0) {
        assistantMessage = (reply.content || '').trim();
        break;
      }
      console.log(`[Assistants POC] Round ${round + 1}: ${toolCalls.length} tool call(s):`, toolCalls.map((t)=>t.function?.name).join(', '));
      conversation.push({ role: 'assistant', content: reply.content ?? null, tool_calls: toolCalls });
      for (const toolCall of toolCalls){
        let output;
        try {
          output = await handleToolCall(toolCall);
        } catch (toolError) {
          console.error('[Assistants POC] Tool threw:', toolCall.function?.name, toolError);
          output = `Error: ${(toolError as any)?.message || toolError}`;
        }
        conversation.push({
          role: 'tool',
    ...
    if (!assistantMessage) {
      // Rounds exhausted mid-tool-chain (or an empty reply): ask for a plain summary of
      // what was done so the user is not left with nothing.
      const summary = await openai.chat.completions.create({
        model: assistantConfig.model,
        messages: [...conversation, { role: 'user', content: 'Summarise in one or two sentences what you have done so far and what is still needed.' }],
        tool_choice: 'none',
        tools: assistantConfig.tools as any
      });
      assistantMessage = (summary.choices?.[0]?.message?.content || '').trim() || 'No response generated';
    }
