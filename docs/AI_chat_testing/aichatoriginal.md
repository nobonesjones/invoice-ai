      ...
      if (name === 'create_invoice') {
        ...
        if (invoiceError) {
          ...
          if (invoiceError.code === '23505' && invoiceError.message.includes('idx_invoices_request_id')) {
            ...
            const { data: existingInvoice, error: fetchError } = await supabase.from('invoices').select('*').eq('request_id', deduplicationKey1).single();
            if (fetchError) {
            ...
            const { data: existingLineItems } = await supabase.from('invoice_line_items').select('*').eq('invoice_id', existingInvoice.id).order('created_at', {
              ascending: true
            });
            // Get client data
            let clientData = null;
            if (existingInvoice.client_id) {
              const { data: client } = await supabase.from('clients').select('*').eq('id', existingInvoice.client_id).single();
              clientData = client;
            }
