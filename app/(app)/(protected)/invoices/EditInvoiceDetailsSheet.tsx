...
import SetDueDateSheet, {
	DueDateOption,
	DUE_DATE_OPTIONS,
} from "./SetDueDateSheet";
...
>(({ initialDetails, onSave, onClose }, ref) => {
	...
	const [selectedDueDateType, setSelectedDueDateType] = useState(
		initialDetails?.dueDateType || "on_receipt",
	);
	const [selectedCustomDueDate, setSelectedCustomDueDate] =
		useState<Date | null>(
			initialDetails?.customDueDate
				? typeof initialDetails.customDueDate === "string"
					? parseISO(initialDetails.customDueDate)
					: initialDetails.customDueDate
				: null,
		);

	const getInitialDueDateDisplay = () => {
		if (initialDetails?.dueDateDisplayLabel)
			return initialDetails.dueDateDisplayLabel;
		if (initialDetails?.customDueDate) {
			const customDate =
				typeof initialDetails.customDueDate === "string"
					? parseISO(initialDetails.customDueDate)
					: initialDetails.customDueDate;
		...
		const initialOption = DUE_DATE_OPTIONS.find(
			(opt: DueDateOption) => opt.type === initialDetails?.dueDateType,
		);
		return initialOption ? initialOption.label : "On receipt";
	};
	const [dueDateDisplay, setDueDateDisplay] = useState(
		getInitialDueDateDisplay(),
	);
	...
	const handleInternalSave = () => {
		...
		const updatedDetails: InvoiceDetailsData = {
			...
			dueDateDisplayLabel: dueDateDisplay,
		};
	...
	const handleDueDateSelect = (
		...
		if (type === "custom" && customDate) {
			setSelectedCustomDueDate(customDate);
			setDueDateDisplay(displayLabel || format(customDate, "MMM d, yyyy"));
		} else if (type === "custom" && !customDate) {
			showCustomDatePicker(); // Prompt for date if 'custom' is selected but no date given
			// Keep current custom date for now, or clear it: setSelectedCustomDueDate(null);
		} else {
			setSelectedCustomDueDate(null); // Clear custom date if a non-custom type is chosen
			const option = DUE_DATE_OPTIONS.find((opt) => opt.type === type);
			setDueDateDisplay(option ? option.label : "Select Due Date");
	...
	const handleConfirmCustomDueDate = (date: Date) => {
		...
		setDueDateDisplay(format(date, "MMM d, yyyy"));
	...
	return (
		<BottomSheetModal
			...
			name="editInvoiceDetailsModal" // Give it a name
			stackBehavior="push"
			index={0} // Start open
			...
			>
				<View style={styles.inputGroupContainer}>
					<View style={styles.inputRow}>
						<Text style={styles.inputLabelText}>Invoice #</Text>
					...
					<TouchableOpacity
						...
						disabled={isLoading}
					>
						<Text style={styles.inputLabelText}>Due Date</Text>
						<View style={styles.inputValueArea}>
							<CalendarDays
								size={18}
								style={[
									styles.iconStyle,
									dueDateDisplay !== "Select Due Date" &&
										dueDateDisplay !== "On receipt" &&
										styles.primaryIcon,
								]}
							/>
							<Text style={[styles.pressableRowText, { marginLeft: 8 }]}>
				...
				<View style={styles.inputGroupContainer}>
					<View style={styles.inputRow}>
						<Text style={styles.inputLabelText}>PO Number</Text>
						<View style={styles.inputValueArea}>
							<BottomSheetTextInput
								style={styles.textInputStyled}
								value={poNumber}
					...
					<View
						style={[styles.inputRow_last, styles.multilineTextInputContainer]}
					>
