...
import { X, Search, PlusCircle, UserPlus, User as UserIcon } from 'lucide-react-native';
...
>(({ onClientSelect, onClose }, ref) => {
  ...
  const renderEmptyComponent = () => {
    if (loading) return <ActivityIndicator size="large" color={themeColors.primary} style={{ marginTop: 20 }} />;
    if (error) return <Text style={styles.errorText}>{error}</Text>;
    if (!clients.length) return <Text style={styles.emptyText}>No clients found. Add one to get started!</Text>;
    if (!filteredClients.length && searchTerm) return <Text style={styles.emptyText}>No clients match "{searchTerm}"</Text>;
    return null;
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.searchContainer}>
        <Search size={20} color={themeColors.mutedForeground} style={styles.searchIcon} />
        <BottomSheetTextInput
      ...
      <View style={styles.actionButtonsContainer}>
        <TouchableOpacity style={styles.actionButton} onPress={() => console.log('Add from contacts pressed')}>
          <PlusCircle size={20} color={themeColors.primary} style={styles.actionButtonIcon} />
          <Text style={styles.actionButtonText}>Add from Device Contacts</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionButton} onPress={() => console.log('Create new client pressed')}>
          <UserPlus size={20} color={themeColors.primary} style={styles.actionButtonIcon} />
