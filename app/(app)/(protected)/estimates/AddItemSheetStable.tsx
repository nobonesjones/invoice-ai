...
const AddItemSheetStable = forwardRef<AddItemSheetStableRef, AddItemSheetStableProps>(({ onItemFromFormSaved, currencyCode = 'USD' }, ref) => {
  ...
  React.useImperativeHandle(ref, () => ({
    present: () => {
      // Open the parent sheet first (like Clients modal),
      // then fetch saved items shortly after.
  ...
  return (
    <BottomSheetModal
      ...
      onChange={(i) => {
        ...
        setIsOpen(open);
        if (open) {
          // Refresh list when the sheet becomes visible
          fetchSavedItems();
        }
      }}
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View>
                ...
                {!fetchError && filteredSavedItems.length === 0 && searchQuery === '' && (
